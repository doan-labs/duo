...
- **Smoothness = per-frame complete tiles + eased camera.** In the bake, `await map.once('idle')` before each screenshot and ease the camera with `easeInOutCubic`. `preserveDrawingBuffer:true`, `fadeDuration:0`, large `maxTileCacheSize`.
