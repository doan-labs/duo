...
| A mirrored strip of app content at the cover's right edge | A composited element clipped at the display edge can raster a mirrored copy of itself at the boundary (the same family as the backdrop-filter mirror in `ramp`), especially under SwiftShader; hiding the element's iframe removes it. It is a raster artifact, not layout - check a fresh mount before tracing app code |
