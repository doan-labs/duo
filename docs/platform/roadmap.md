...
| Device events | Implemented locally, 2026-09-25 / needs native verification | Volume, Camera Control, side, orientation and read-only switches through `os.device.on`; Tauri WKWebView, split-display listeners and hidden-view orientation throttling are unverified; haptics, battery and writable switches are not designed ([decision 95](../decisions.md)) |
