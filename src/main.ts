...
import { screen } from './os/screen.ts'
import { toggles } from './os/springboard/toggles.ts'
...
// finger on a button (src/buttons.ts) and carries every mesh and panel.
...
// Screen textures, baked by src/os/screen.ts (metres in, canvas out).
...
// wide with the same margin — both src/hud.tsx) plus PAD of slack, and the bare
...
// The buttons on the frame have keys too, in src/buttons.ts.
